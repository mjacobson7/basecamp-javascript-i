...
// CODE HERE
var wildNameReturn = (name) => 'wild ' + name;
...
// CALLBACKS
// Here we will write a function that takes in a callback as a parameter. You must write the callback function to make the 
//existing function work properly.
// EXAMPLE
...
// CODE HERE
var first = (nameArr, callback) =>  callback(nameArr[0]);
...
// CODE HERE
var last = (nameArr, callback) => callback(nameArr.pop());
...
// CODE HERE
var sizer = (num1, num2, callback) => num1 > num2 ? callback(num1) : callback(num2);
...
shortNames = friends.filter(friend => friend.length < 4);

// #13 here we have created a NickNameMaker function that will shorten names to the begninning two Characters so 'Cam' 
//will be 'CaCa. Using the nickNameMaker function, map over the previously created friends array to return the friends 
//nick names and assign it to 'shorty'.

var nickNameMaker = (name) => {
  return name.split('').slice(0,2).join('') + name.split('').slice(0,2).join('');
}

var shorty = friends.map(friend => nickNameMaker(friend)); // CODE HERE
...
// CODE HERE
var plusThree = numbers.map(number => number + 3);
...
// CODE HERE
goodPeople.forEach(person => person.family = true);